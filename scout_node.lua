--[[
    ...
    Post-launch fixes:
        ...
        - isEquippable() no longer rejects pickaxes with Forge tag NBT
          (shown as "(alt)" in tooltips). Only damage or enchantments block
          equip, matching CC:Tweaked's real restriction.
        - isTool() guard on all dropDown() calls: pickaxe and geo_scanner
          are never dropped into the dump chest or thrown away at base.
        - recoverSpiral() adds a climb-over manoeuvre: when all 6 directions
          fail (e.g. create:item_vault blocking path), steps up one block
          and tries all 4 horizontal directions from above the obstacle.
        - scanAround and gpsSyncPos forward-declared so nav code can call
          them before their definition in the file (was crashing with nil).
        - state_PARKED navigates to the overseer-assigned park slot inside
          the setpark rectangle instead of stopping in place.
        - GPS re-sync every 8 blocks during mining (was nav-only at 16).
          Calibration verifies saved heading with a live GPS step on reboot.
        - GOTO state now does a full vein sweep (all 6 faces, repeat until
          no more matching ore) instead of arriving and reporting one block.
